...
import pandas as pd
import torch
from pplm import PplmTokenizer
...
class DirectedEvolution:
    ...
    def __init__(
        ...
        if self.preserved_regions is not None:
            for start, end in self.preserved_regions:
                if end - start < 0:
                    raise ValueError(
                        "Preserved regions must be at least 1 amino acid long"
                    )
        ...
        # will need to update to support non-cannonicals
        # self.canonical_chain_tokenizer = PplmTokenizer()
    ...
    def _compute_gradients(
        self, ohs: List[torch.Tensor], PoE: torch.Tensor
    ) -> torch.Tensor:
    ...
    def _get_variants_and_scores(self) -> Tuple[List[str], np.ndarray]:
        ...
        if self.output == "last":
            ...
            for i in range(len(self.chains_oh_history)):
                variants += [
                    self.canonical_chain_tokenizer.decode(self.chains_oh_history[i])
                ]
            scores = torch.stack(self.PoE_history).numpy()
        elif self.output == "best":
            best_idxs = torch.stack(self.PoE_history).argmax(0)
            chains_oh_history = torch.stack(
                self.chains_oh_history
            )  # [n_steps, n_chains, seq_len, n_tokens]
            variants = self.canonical_chain_tokenizer.decode(
                torch.stack(
                    [
                        chains_oh_history[best_idxs[i], i]
    ...
    def __call__(self) -> Tuple[List[str], np.ndarray]:
        ...
        for i in range(self.n_steps):
            ###### sample path length
            U = torch.randint(
                1, 2 * self.max_pas_path_length, size=(self.parallel_chains, 1)
            )
            ...
            with torch.no_grad():
                for step in range(max_u):
                    ...
                    score_change = grad_x - (grad_x * cur_chains_oh).sum(-1).unsqueeze(
                        -1
                    )
                    traj_list += [cur_chains_oh]
                    approx_forward_expert_change = (
                        score_change.reshape(self.parallel_chains, -1) / 2
                    )
                    ...
                    cd_forward = (
                        torch.distributions.one_hot_categorical.OneHotCategorical(
                            probs=utils.safe_logits_to_probs(
                                approx_forward_expert_change
                            )
                        )
                    )
            ...
            with torch.no_grad():
                ...
                reverse_score_change = grad_y.unsqueeze(1) - (
                    grad_y.unsqueeze(1) * traj
                ).sum(-1).unsqueeze(-1)
                reverse_score_change = (
                    reverse_score_change.reshape(self.parallel_chains, max_u, -1) / 2.0
                )
                log_ratio = 0
                for id in range(len(onehot_Idx)):
                    cd_reverse = (
                        torch.distributions.one_hot_categorical.OneHotCategorical(
                            probs=utils.safe_logits_to_probs(
                                reverse_score_change[:, id]
                            )
                        )
                    )
                    log_ratio += u_mask[:, id] * (
                        cd_reverse.log_prob(onehot_Idx[id])
                        - forward_categoricals[id].log_prob(onehot_Idx[id])
                    )
                ...
                accepted = (
                    (log_acc.exp() >= torch.rand_like(log_acc))
                    .float()
                    .view(-1, *([1] * x_rank))
                )
            ...
            if self.verbose:
                ...
                for idx, variant in enumerate(x_strs):
                    print(
                        f">step {i}, chain {idx}, acceptance rate: {log_acc[idx].exp().item():.4f}, product of experts score: {PoE[idx]:.4f}"
                    )
